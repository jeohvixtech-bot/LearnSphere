...
public class BookingClass
{
    ...
    // Whether this individual session actually happened. A tutor is only ever paid for a
    // session that is BOTH delivered and covered by a paid invoice, so this is the gate on
    // real money leaving the platform — a booking-level status is too coarse, because a
    // month's four sessions are paid for together but delivered one at a time.
    //
